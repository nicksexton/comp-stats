...
function y = norm_proportional (theta)
  y = exp(-1 * theta .^ 2 + 2 .* theta);
end

function y = norm_withparams (theta, sigma, mu)
  y = 1/sigma * exp((-1 * theta .^ 2) + (2 * mu .* theta) - (mu .^ 2) );
end
...
sigma = 0.5 # std dev of normal proposal density
thetamin  = -30; thetamax = 30; # define a range for starting values

target_mu = -5# mean of target (normal);
target_sigma = 20 # sd of target (normal);
...
while t < T # iterate until we have T samples
  ...
  alpha = min ([ 1 (norm_withparams( theta_star, target_sigma, ...
				    target_mu) / 
		    norm_withparams(theta(t-1), target_sigma, target_mu ))]);
...
y = norm_withparams (thetabins, target_sigma, target_mu); # use normpdf instead?
hold on;
